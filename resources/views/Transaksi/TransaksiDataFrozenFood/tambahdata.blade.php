...
<section class="content">
    <div class="card card-danger">
        <div class="card-header">
            <h5 class="card-title">
                <i class="fa fa-edit"></i>
                Tambah Data Transaksi Frozen Food
            </h5>
        </div>
        <div class="card-body">
          <div class="table-responsive">
          
            <form method="POST" action="{{route('simpantransaksifrozenfood')}}" >
              @csrf
              <table class="table table-bordered" id="dynamicAddRemove">
                <thead>
                <tr>
                    <th>Nama Makanan Food</th>
                    <th>Harga</th>
                    <th>Jumlah</th>
                    <th>Total</th>
                </tr>
              ...
              <tbody>
                <tr>
                    <td>
                      <select name="addMoreInputFields[0][nama_makanan]" id="nama_makanan" class="form-control nama_makanan" required>
                        ...
                        <option value="{{ $b->nama_makanan }}" data-harga="{{$b->harga}}">{{$b->nama_makanan}}</option>
                        @endforeach
                      </select>
                    </td>
                    <td>
                ...
                <div class="form-group text-right">
                  <a href="/transaksi/data-frozen-food" title="Kembali" class="btn btn-primary"><i class="fa fa-back"></i>Kembali</a>
                  <button type="submit" class="btn btn-success"><i class="fa fa-save"></i> Simpan Data</button>
                </div>
                </form>
