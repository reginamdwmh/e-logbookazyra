...
class PesanController extends Controller
{
    ...
    public function selesaihistory($id_pesanan)
    {
        $users = UsersModel::select('*')
            ->get();
        $pesanan = PesananModel::where('id_pesanan', $id_pesanan)->first();
        $pesanan->status = 4;
        $pesanan->update();
        ...
        $data_penjualan = PesananModel::where('id_pesanan', $id_pesanan)->first();
        // dd($data_penjualan_detail);
        Alert::success('Success', 'Pesanan diterima');
        return redirect()->back()->with(['users' => $users, 'data_penjualan' => $data_penjualan, 'data_penjualan_detail' => $data_penjualan_detail]);
    }
}
