...
class LaporanDataPenjualanMakananController extends Controller
{
    ...
    public function cetaklaporantransaksipenjualanmakanan($tglawal, $tglakhir)
    {
        ...
        // return view('Laporan.LaporanDataPenjualanMakanan.laporan', ['tanggal' => $tanggal,'users' => $users],compact('tglawal','tglakhir'));
        $user = Auth::user();
        if ($user->role == 'admin') {
            $pdf = PDF::loadView('admin.Laporan.LaporanDataPenjualanMakanan.laporan', ['tanggal' => $tanggal, 'users' => $users], compact('tglawal', 'tglakhir'));
            return $pdf->stream('Laporan-Data-Transaksi-Penjualan-Makanan-Offline.pdf');
        } else {
